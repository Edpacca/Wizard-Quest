...
function Wizard:init(map)
    ...
    self.texture = love.graphics.newImage('Graphics/Wizard_spritesheet.png')
    ...
    self.behaviours = {
        ...
        ['casting_fireball'] = function(dt)
            ...
            self.fireball:spawn_fireball(self.x + self.xOffset, self.y + self.yOffset, aim_x, aim_y)
        ...
        ['casting_frost_release'] = function(dt)
            ...
            local aim_y = MOUSE_Y
            CASTING_FROST = true
            self.frostray:spawn_frostray(self.x, self.y, aim_x, aim_y)
...
function Wizard:target_tiles()
    ...
    if self:check_specific_tile(12, 30) then
    ...
    if self:check_specific_tile(49, 30) then
...
function Wizard:render()
    ...
    if CASTING_FROST == true then
        self.frostray:frostray_render(self.x + self.xOffset, self.y + self.yOffset)
    end
    ...
    love.graphics.print("ICE_ACTIVE: " ..tostring(ICE_ACTIVE), self.map.camX + 20, self.map.camY + 230  )
    love.graphics.print("CASTING_FROST: " ..tostring(CASTING_FROST), self.map.camX + 20, self.map.camY + 240  )
