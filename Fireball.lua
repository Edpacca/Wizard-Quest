...
function Fireball:init(map)
    ...
    self.spell_texture = love.graphics.newImage('Graphics/Casting/Fireball/fireball_single.png')
    self.fb_height = 8
    self.fb_width = 8
...
function Fireball:spawn_fireball(x, y, aim_x, aim_y)
    ...
    FIREBALLS[table.getn(FIREBALLS) + 1] = {nfb_x = x, nfb_y = y, img = self.spell_texture, 
        ...
        y_speed = y_vector / (math.abs(x_vector) + math.abs(y_vector))}
...
function Fireball:fireball_render()
    for i, new_fireball in ipairs(FIREBALLS) do
        love.graphics.draw(FIREBALLS[i].img, FIREBALLS[i].nfb_x, FIREBALLS[i].nfb_y, 0.5, fireball_scale, fireball_scale)
